/**
 * 
 *    Understanding tables with relationships 
 */
const { pgClient, prisma } = require("../db")
...
async function main() {
    ...
    // create users table 
    // create photos table and add relation to users 
    // create comments table add relation to users and photos 
    // create like table add relation to users and photos
    await prisma.$queryRaw`
        ...
        -- insert some photos to table photos 
        -- insert into photos (url,user_id) values 
        -- ('url1.com',1),
        -- ('url2.com',2),
        -- ('url3.com',3),
        -- ('url4.com',4)
        -- ('url500.com',500)  -- throws error non existing user
        -- ('delete.com',5)

        -- try to delete a user who have a relation in photos 
        -- delete from users where id = 5   -- throws error 

        -- create table with cascade relation option 
        -- create table if not exists photos (
        --     id serial primary key,
        --     url varchar(200),
        ...
        -- delete a user that have a relation in photos 
        -- delete from users where id = 5   -- all photos will be too deleted 

        -- create table with set null relation option 
        -- create table if not exists photos (
        --     id serial primary key,
        --     url varchar(200),
        ...
        -- delete a user that have a relation in photos 
        -- delete from users where id = 5   -- user id in photos will be set to null 
...
--         ('Minima dolorem reiciendis excepturi culpa sapiente eos deserunt ut.', 3, 3);
    ...
    console.log(
        await prisma.$queryRaw`
            ...
            -- select contents,username from comments join users on users.id = comments.user_id limit  2
            -- if joining table contains same column name need to aliase or specify like table.id 
            -- select users.id as user_id,comments.id as comment_id from comments join users on users.id = comments.user_id limit  2

-- 4 types of join  
-- 1. inner join aka join (it will take values from both table only when it have relation otherwise data will be dopped)
-- 2. left join (all data from left table, if no relation in right table it's columns will be set as null and right table data willbe dropped if it have any relation to left table)

-- select * from users left join comments on users.id = comments.user_id 

-- 3. right join (same as left join but right table have the privilage)
